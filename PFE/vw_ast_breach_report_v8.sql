-- =====================================================================
-- vw_ast_breach_report   (v8 — + Sector mapping, REQ 1.1)
...
SELECT
    ...
    -- (+) v8 REQ 1.1: mapped 5-bucket Sector for grouping. Breach list shows granular
    --     Industry under Sector headers. HF=SIC 7298 IS resolvable here (sic_code on
    --     test_ats_summary), unlike the portfolio view.
    CASE
      WHEN ats_summary.sic_code = '7298'                       THEN 'Hedge Funds'
      WHEN upper(ats_summary.sic_industry) LIKE '%BANK%'       THEN 'Banks'
      WHEN upper(ats_summary.sic_industry) LIKE '%GOV%'        THEN 'Government'
      WHEN upper(ats_summary.sic_industry) LIKE '%FINANC%'     THEN 'Financial'
      ELSE 'Corporates'
    END                            AS Sector,            -- (+) v8 REQ 1.1
