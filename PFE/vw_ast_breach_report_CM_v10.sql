-- =====================================================================
-- vw_ast_breach_report   (v9 — Sector aligned to coarse industry + HF=7298)
...
CREATE OR REPLACE VIEW `d4001-centralus-tdvip-creditrisk`.`xvala_core`.`vw_ast_breach_report` (
  ...
  Is_Breached COMMENT '(+) v10: Y if ANY of the 6 per-bucket Excess_Breach flags is TRUE (the analyst breach definition). Now a column, not a row filter — dashboard filters on this for the breach list.',
...
WITH breach_lines AS (
    -- (~) v10 DE-FILTERED: was filtered to breached lines only. Now returns ALL lines;
    --     the 6 Excess_Breach flags are CARRIED FORWARD so Is_Breached can be derived
    --     downstream. Breach becomes a COLUMN (and a dashboard filter), not a row filter.
    --     This makes the view serve general line queries + approaching + breached.
    SELECT
        Line,
        Long_Name,
        ...
        Max_Exp_Time_Bucket,
        Max_Scenario_Name,
        -- (+) v10: the 6 per-bucket breach flags, carried for the Is_Breached column
    ...
    FROM `d4001-centralus-tdvip-creditrisk`.`xvala_core`.`pfe_asts` AS ast
    WHERE regexp_replace(CAST(ast.business_date AS STRING),'-','') = '20260430'  -- datatype-agnostic
      AND ast.No_Line_Indicator = 'False'                  -- population filter (kept); breach filter REMOVED
),
...
SELECT
    ...
    -- (+) v10: Is_Breached — the analyst's breach definition (ANY of 6 bucket flags),
    --     now a COLUMN not a row filter. Dashboard filters on this for the breach list.
    CASE WHEN ast.`0_3_mo_Excess_Breach`='TRUE' OR ast.`3_12_mo_Excess_Breach`='TRUE'
           OR ast.`1_2_Yr_Excess_Breach`='TRUE' OR ast.`2_5_Yr_Excess_Breach`='TRUE'
           OR ast.`5_10_Yr_Excess_Breach`='TRUE' OR ast.`10_50_Yr_Excess_Breach`='TRUE'
         THEN 'Y' ELSE 'N' END     AS Is_Breached,
