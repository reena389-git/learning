-- =============================================================================
-- portfolio_aggregation_v5.sql   (REQUIREMENTS-ALIGNED — sector map + no-line filter)
--
...
SELECT
  ...
  -- (REQ 1.1) mapped 5-bucket SECTOR. Banks/Financial/Government matched on text;
  -- Hedge Funds needs SIC 7298 (not on ats_summary) -> flagged; else Corporates.
  CASE
    WHEN upper(n.industry) LIKE '%BANK%'                              THEN 'Banks'
    WHEN upper(n.industry) LIKE '%GOV%' OR upper(n.sic_industry) LIKE '%GOV%'
                                                                     THEN 'Government'
    WHEN upper(n.industry) LIKE '%FINANC%' OR upper(n.sic_industry) LIKE '%FINANC%'
                                                                     THEN 'Financial'
    -- Hedge Funds = SIC 7298 — UNRESOLVABLE here (no sic_code on ats_summary).
    -- WHEN n.sic_code = '7298' THEN 'Hedge Funds'   <-- enable when sic_code available
    ELSE 'Corporates'
  END AS sector,                                                  -- (+) REQ 1.1
  CASE
