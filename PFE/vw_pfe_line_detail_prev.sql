...
SELECT
    ...
    -- (+) v11.2: 8 scenario maxima at LINE grain (DOUBLE, additive across lines).
...
LEFT JOIN (
        ...
        SELECT Line, sic_industry, industry, brr, sic_code, otc_sft,
               -- (+) v11.2: 8 scenario maxima (line grain, DOUBLE) carried through so
               --     this view can serve as the PORTFOLIO source. GREATEST happens at
               --     CP grain in Mosaic (sum each across lines, then max) — NOT here.
               cartor_max, zero_max, c_25_max, c_75_max,
               str75_max, one_max, prod_max, strmpr025_max
...
-- VALIDATION
-- =====================================================================
-- V-ROLLUP  (+) v11.2 THE PORTFOLIO TEST: proves this view can REPLACE the
--           separate portfolio view. Rolls the 8 scenario columns up to
--           counterparty TWO ways and shows the gap:
--             correct_stress = GREATEST( SUM(each scenario across the CP's lines) )
--                              -> sum FIRST, then max-of-8  (the right answer)
--             naive_stress   = SUM( each line's own Stress_PFE )
--                              -> sums pre-collapsed line maxima (overstates)
--           If correct_stress is sensible and generally <= naive_stress by the
--           validated ~0.2-0.6%, the two-step works from these columns and the
--           portfolio view can retire: portfolio = this view aggregated in Mosaic
--           with a level metric doing exactly this GREATEST(SUM(...)). If Mosaic
--           can't express the max-of-8-sums reliably, keep a THIN rollup view
--           that runs just this SELECT.
SELECT
    Counterparty_Name,
    GREATEST(
...
ORDER BY correct_stress_mm DESC
LIMIT 20;
