...
- **FR-3.2 natural** — the **minimal set of business columns that is a valid key** under FR-3.7/FR-3.8 (no nulls, no duplicate rows); this expresses the **grain**.
...
- **FR-3.4 alternate** — a column/set that is unique but is **not** the chosen primary key. A column unique only on its *non-null* subset is at most an alternate key, never a PK.
...
- **FR-3.7 Duplicate-row guard (run first).** Before proposing any key, detect **full-row duplicates** (`count(*) − count(distinct *)`) and report the count. A table with duplicate rows has **no valid key as-loaded** — the profiler must NOT silently emit a PK the raw data violates. Instead it (a) reports the duplicate-row count, (b) evaluates candidacy on the **de-duplicated** set and flags any proposal **"valid only after de-duplication (N dup rows)"**, and (c) distinguishes **exact full-row duplicates** (usually a load defect → `SELECT DISTINCT`) from **key-only duplicates** (same candidate-key values, differing other columns → a genuine grain/SCD problem, more serious).
- **FR-3.8 Null guard.** A primary-key column — and **every member** of a composite key — must be **NOT NULL**. Because `count(distinct col)` ignores nulls, the uniqueness test must be **`count(col) = count(*)` AND `count(distinct col) = count(*)`** (no nulls *and* no duplicate values), so nullable columns cannot masquerade as unique. Exclude all-null and high-null columns from the key search entirely; report per-candidate **null counts** as exception support.
- **FR-3.9 Surrogate validation.** Never assume a `*_key` / `BIGINT` column is a valid surrogate. **Verify** it is unique, non-null, and free of duplicate rows; if any check fails, flag it as a **load defect** and do not propose it as the PK. (A surrogate with duplicates or nulls is a pipeline bug, not a key.)
- **FR-3.10 Reporting.** Every key proposal carries a validity verdict — **valid / valid-after-dedup / invalid** — with the duplicate-row count and per-member null count attached (per FR-10.2 observed-vs-suggested + exception counts).
...
- **AC-10** On a table containing duplicate rows or nullable candidate columns, the profiler does **not** emit a false PK: it reports the duplicate-row count and per-candidate null counts, marks any key "valid-after-dedup" or "invalid" as appropriate, and validates surrogate keys for uniqueness + non-null + no duplicates before proposing them.
